...
import os
import torch
from .NoisePrintPlus.DnCNN import make_net
from PIL import Image


@TRANSFORMS.register_module()
class NPPTest(BaseTransform):
    
    def __init__(self):
        
        abs_path = os.path.dirname(os.path.abspath(__file__))
        default_npp_path = os.path.join(abs_path, 'NoisePrintPlus/npp.pth')
        self.NPP_path = os.environ.get('T07_MUN_NPP_PATH', default_npp_path)
        self.device = torch.device(
            os.environ.get('T07_MUN_DEVICE', 'cuda:0' if torch.cuda.is_available() else 'cpu')
        )

        num_levels = 17
        out_channel = 1
        self.npp_net = make_net(3, kernels=[3, ] * num_levels,
        ...
        self.npp_net = torch.load(self.NPP_path, map_location=self.device)
        for _, v in self.npp_net.named_parameters():
           v.requires_grad=False

        
        self.npp_net.to(self.device)
    ...
    def transform(self, results: dict) -> dict:
        ...
        npp_rgb = self.npp_net(rgb)
        npp_rgb = torch.tile(npp_rgb, (3, 1, 1))

        npp = torch.squeeze(npp_rgb, 0)[0]
        npp = npp.cpu().numpy()
...
@TRANSFORMS.register_module()
class NPPTrain(BaseTransform):
    def __init__(self):
        self.npp_path = '/path_to_your_dataset/npp_dir/training/'

    def transform(self, results: dict) -> dict:
        img_path = results['img_path']
        img_name = img_path.split('/')[-1] + '.npz'
        npp_path = os.path.join(self.npp_path, img_name)

        result = np.load(npp_path)
        noisepr = result['np++']
...
@TRANSFORMS.register_module()
class NPPValidation(NPPTrain):

    def __init__(self):
        self.npp_path = '/path_to_your_dataset/npp_dir/validation/'


@TRANSFORMS.register_module()
class NPPResize(MMCV_Resize):
    ...
    def _resize_npp(self, results: dict) -> None:
        if results.get('npp', None) is not None:
            if self.keep_ratio:
                npp_seg = mmcv.imrescale(
                        results['npp'],
                        results['scale'],
                        interpolation='nearest',
                        backend=self.backend)
            else:
                npp_seg = mmcv.imresize(
                        results['npp'],
                        results['scale'],
                        interpolation='nearest',
                        backend=self.backend)
    ...
    def transform(self, results: dict) -> dict:
        ...
        self._resize_npp(results)
        return results


@TRANSFORMS.register_module()
class NPPRandomCrop(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        results['npp'] = self.crop(results['npp'], crop_bbox)
...
@TRANSFORMS.register_module()
class NPPRandomFlip(MMCV_RandomFlip):
    ...
    def _flip(self, results: dict) -> None:
        ...
        if results.get('npp', None) is not None:
            results['npp'] = self._flip_seg_map(
                results['npp'], direction=results['flip_direction']).copy()
...
@TRANSFORMS.register_module()
class NPPResizeToMultiple(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        npp_seg = results['npp']
        npp_seg = mmcv.imresize_to_multiple(
                npp_seg,
                self.size_divisor,
                scale_factor=1,
                interpolation='nearest')
...
class GaussianBlur(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        img = results['img']
        blur_img = cv2.GaussianBlur(img, (self.k, self.k), 0, 0)
        results['img'] = blur_img
        return results
        

@TRANSFORMS.register_module()
class JPEGCompress(BaseTransform):
    def __init__(self):
        self.q = None
    ...
    def transform(self, results: dict) -> dict:
        ...
        img = results['img']
        name = results['img_path']
        name = name.split('/')[-1]
        save_path = os.path.join('/path_to_your_temp_folder/temp/', name)
        cv2.imwrite(save_path, img, [cv2.IMWRITE_JPEG_QUALITY, self.q])
        jpeg_img = cv2.imread(save_path)
        results['img'] = jpeg_img
        os.remove(save_path)
        return results
...
################调整test数据###################
@TRANSFORMS.register_module()
class AdjustPad(BaseTransform):
    """adjust_pad the image .
    ...
    Args:
        adjust_pad_size (Union[int, Tuple[int, int]]):  Expected size after adjust
            with the format of (h, w). If set to an integer, then adjusting
            width and height are equal to this integer.
        ignore_index (int): The label index to be ignored. Default: 255
    """

    def __init__(self,
                 adjust_pad_size: Union[int, Tuple[int, int]],
                 ignore_index: int = 255):
        super().__init__()
        assert isinstance(adjust_pad_size, int) or (
            isinstance(adjust_pad_size, tuple) and len(adjust_pad_size) == 2
        ), 'The expected adjust_pad_size is an integer, or a tuple containing two '
        'intergers'

        if isinstance(adjust_pad_size, int):
            adjust_pad_size = (adjust_pad_size, adjust_pad_size)
        assert adjust_pad_size[0] > 0 and adjust_pad_size[1] > 0
        self.adjust_pad_size = adjust_pad_size
        #self.cat_max_ratio = cat_max_ratio
        self.ignore_index = ignore_index

    @cache_randomness
    def adjustimage_size(self, results: Dict) -> np.ndarray:
        print("adjustimage_size_start")
        img = results['img']
        gt_seg_map = results.get('gt_seg_map', None)

        target_height, target_width = self.adjust_pad_size

        # 调整宽度
        if img.shape[1] != target_width:
            if img.shape[1] > target_width:
                # 裁剪
                offset_w = (img.shape[1] - target_width) // 2
                img = img[:, offset_w:offset_w + target_width, ...]
                if gt_seg_map is not None:
                    gt_seg_map = gt_seg_map[:, offset_w:offset_w + target_width]
            else:
                # 填充
                ...
                if gt_seg_map is not None:
                    gt_seg_map = np.pad(gt_seg_map, ((0, 0), (left_pad, right_pad)), mode='constant',
        ...
        if img.shape[0] != target_height:
            if img.shape[0] > target_height:
                # 裁剪
                offset_h = (img.shape[0] - target_height) // 2
                img = img[offset_h:offset_h + target_height, :, ...]
                if gt_seg_map is not None:
                    gt_seg_map = gt_seg_map[offset_h:offset_h + target_height, :]
            else:
                # 填充
                ...
                if gt_seg_map is not None:
                    gt_seg_map = np.pad(gt_seg_map, ((top_pad, bottom_pad), (0, 0)), mode='constant',
                                        constant_values=self.ignore_index)
        ...
        if gt_seg_map is not None:
            ...
            # 返回调整后的图像（或根据需要返回其他信息）
        return img

    def adjustimage_npp(self, results: Dict) -> np.ndarray:
        npp = results['npp']
        # gt_seg_map = results.get('gt_seg_map', None)
        ...
        # 调整宽度
        if npp.shape[1] != target_width:
            if npp.shape[1] > target_width:
                # 裁剪
                offset_w = (npp.shape[1] - target_width) // 2
                npp = npp[:, offset_w:offset_w + target_width, ...]
                # if gt_seg_map is not None:
                #     gt_seg_map = gt_seg_map[:, offset_w:offset_w + target_width]
            else:
                # 填充
                ...
                left_pad = pad_width // 2
                right_pad = pad_width - left_pad
                npp = np.pad(npp, ((0, 0), (left_pad, right_pad), (0, 0)), mode='constant', constant_values=0)
                # if gt_seg_map is not None:
                #     gt_seg_map = np.pad(gt_seg_map, ((0, 0), (left_pad, right_pad)), mode='constant',
                #                         constant_values=self.ignore_index)

                    # 调整高度
        if npp.shape[0] != target_height:
            if npp.shape[0] > target_height:
                # 裁剪
                offset_h = (npp.shape[0] - target_height) // 2
                npp = npp[offset_h:offset_h + target_height, :, ...]
                # if gt_seg_map is not None:
                #     gt_seg_map = gt_seg_map[offset_h:offset_h + target_height, :]
            else:
                # 填充
                ...
                top_pad = pad_height // 2
                bottom_pad = pad_height - top_pad
                npp = np.pad(npp, ((top_pad, bottom_pad), (0, 0), (0, 0)), mode='constant', constant_values=0)
                # if gt_seg_map is not None:
                #     gt_seg_map = np.pad(gt_seg_map, ((top_pad, bottom_pad), (0, 0)), mode='constant',
                #                         constant_values=self.ignore_index)
    ...
    def transform(self, results: dict) -> dict:
        """Transform function to adjust images
        ...
        print("原始图像：",img.size)
        #调整图像大小
        img = self.adjustimage_size(results)

###############判断是否要加npp#######################
        npp = results['npp']

        exits = False  # 默认设置 exits 为 False
        try:
            _ = npp
            exits = True
        ...
        if exits:
            npp = self.adjustimage_npp(results)
            results['npp'] = npp
####################################################
    ...
    def __repr__(self):
        return self.__class__.__name__ + f'(adjust_pad_size={self.adjust_pad_size})'
